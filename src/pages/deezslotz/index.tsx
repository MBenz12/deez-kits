...
import {
	...
	getTransactionLogsWithValidation,
	isAdmin,
	playTransaction,
...
const DeezSlotz = React.forwardRef((props, ref) => {
	const connection = new Connection(rpc, {
		...
		confirmTransactionInitialTimeout: confirmTransactionInitialTimeout,
	});
	...
	async function setCommunityBalanceAsync() {
		const tokensBalance = await connection.getParsedTokenAccountsByOwner(communityWallet, {
	...
	const play = async () => {
		...
		try {
			...
			const { txSignature }: any = await playTransaction(
				program,
				provider,
				wallet,
				game_name,
			...
			console.log(
				`result multiplier ${multiplier} equalCount ${equalCount} equalNo ${equalNo} isJackpot ${isJackpot}`
			);
		...
		<div className="slots flex flex-col items-center bg-black min-h-[100vh] lg:p-6 sm:p-4 p-2 font-['Share Tech Mono'] relative">
          ...
          targets.map((target, index) =>
				...
				{prices.map((value, index) => (
					<div
						...
						className={`lg:w-[112px] w-[95px] lg:h-[45px] h-[38px] flex items-center justify-center text-[16px] border-2 rounded-md font-bold cursor-pointer ${
							value === price ? 'text-black bg-[#4AFF2C] border-[#4AFF2C]' : 'text-[#952CFF] border-[#952CFF]'
						}`}
					>
						{value}
						{tokenType ? ' $SKT' : ' SOL'}
			...
			>
				<div
					...
					onMouseOver={() => setBetHovered(true)}
          ...
          ""
			...
			<div className='footer absolute bottom-5 flex items-center'>
				<a href='https://magiceden.io/creators/deezkits' target='_blank' rel='noreferrer' className='me_link'>
					<img src={meIcon} alt='magic-eden-icon' />
					magic eden
				</a>
				<a href='https://twitter.com/deezkits' rel='noreferrer' target='_blank' className='twitter_link'>
					<img src={twitterIcon} alt='twitter-icon' />
					Twitter
				</a>
				<a href='https://discord.gg/deezkits' target='_blank' rel='noreferrer' className='discord_link'>
					<img src={discordIcon} alt='discord-icon' />
					Discord
				</a>

				<a href='https://staking.deezkits.com' rel='noreferrer' className='staking_link'>
					<img src={kitIcon} alt='staking-icon' />
					Staking
				</a>
				<a href='/mutation' rel='noreferrer' className='coinflip_link'>
					<img src={Mutation3} alt='' />
					MUTATION
				</a>
				<a href='https://slotz.deezkits.com' rel='noreferrer' className='staking_link'>
					<span>🎰</span>
					<span>Slotz</span>
				</a>

				<a href='https://coinflip.deezkits.com' rel='noreferrer' className='coinflip_link'>
